...
function listingCardHTML(l) {
  ...
  const media = l.imageBase64 ? `<img src="${l.imageBase64}" alt="${l.title}" style="width:100%;height:100%;object-fit:cover;">` : l.icon;
  return `
...
function openDetailsModal(id, type) {
  ...
  if (type === "event") {
    ...
    const media = item.imageBase64 ? `<img src="${item.imageBase64}" alt="${item.title}" style="width:100%;height:100%;object-fit:cover;">` : item.icon;
    ...
    content.innerHTML = `
      ...
      <div class="card-media" style="border-radius:16px;height:180px;position:relative;">${media}
...
function initCreateListingPage() {
  ...
  if (fileInput) {
    fileInput.addEventListener("change", (event) => {
      ...
      reader.onload = () => {
        listingImageBase64 = reader.result;
        const preview = document.getElementById("mediaPreview");
        if (preview) {
          preview.innerHTML = `<img src="${listingImageBase64}" alt="Listing preview" style="max-width:100%; max-height:160px; object-fit:cover; border-radius:12px;">`;
        }
        toast("Image uploaded and ready to save", "success");
      };
      reader.onerror = () => toast("Image upload failed. Try another file.", "error");
  ...
  if (editId) {
    ...
    if (existing && existing.sellerId === user.id) {
      ...
      listingImageBase64 = existing.imageBase64 || "";
      const preview = document.getElementById("mediaPreview");
      if (preview) {
        preview.innerHTML = listingImageBase64
          ? `<img src="${listingImageBase64}" alt="Listing preview" style="max-width:100%; max-height:160px; object-fit:cover; border-radius:12px;">`
          : chosenIcon;
      }
    ...
    setTimeout(() => {
      if (editId) {
        DB.updateListing(editId, {
          ...
          description: description.value.trim(), icon: chosenIcon, imageBase64: listingImageBase64 || undefined
        });
        ...
        DB.addListing({
          ...
          imageBase64: listingImageBase64 || undefined
        });
